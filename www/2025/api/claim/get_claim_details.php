...
if (isset($_GET['id'])) {
    $num = intval($_GET['id']);
    $query = "SELECT * FROM claimList WHERE num = $num"; 
    ...
    if ($result && mysqli_num_rows($result) > 0) {
		...
		$cSql = "SELECT * FROM `questionnaire` WHERE num = '" . $row['qNum'] . "'";
		$cresult2 = mysqli_query($connect, $cSql);
		
			if ($cresult2) {
				$row3 = mysqli_fetch_assoc($cresult2);

				$response['school1']=$row3['school1'];                  //카드번호
				$response['school2']=$row3['school2'];                         //유효기간
				$response['school3']=$row3['school3'];                       //승인번호
				$response['school4']=$row3['school4'];             //담당자
				$response['school5']=$row3['school5'];		   // 담당자전화번호

				$response['school6']=$row3['school6'];								//계좌번호
				$response['school7']=$row3['school7'];				//은행명
				$response['school8']=$row3['school8'];
				$response['school9']=$row3['school9'];
			} else {	
	...
	/*	$school2 = mysqli_real_escape_string($connect, $row['school2']); // 안전하게 이스케이프 처리
			$query2 = "SELECT * FROM claimList WHERE school2 = '$school2' AND ch = 6";
		...
		if($row['wdate_2']=="0000-00-00"){
			$row['wdate_2']='';
		 }
		 if($row['wdate_3']=="0000-00-00"){
			$row['wdate_3']='';
		 }
