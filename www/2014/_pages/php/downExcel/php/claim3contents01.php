...
//echo "$DNum pageS $pageS";
  ...
  for($_p=0;$_p<$pageS;$_p++){
   ...
   $pdf->AddPage(); 
		 ...
		 switch($_p){
		   case 0 :
			  $pdf->setSourceFile('./hanwha01.pdf'); 
		   break;
			case 1 :
			  $pdf->setSourceFile('./hanwha02.pdf'); 
		   break;
			case 2 :
			  $pdf->setSourceFile('./hanwha03.pdf'); 
		   break;
...
if($_p==0){
	...
	//$data[today3]=iconv("utf-8","euc-kr",$now_time2);

	$now_time4=explode('-',$now_time);


	//echo $now_time2;
	$data[today3_1]=$now_time4[0];
	$data[today3_2]=$now_time4[1];
	$data[today3_3]=$now_time4[2];
	$data[gabunho]=$row[gabunho];	//가계약 번호 
	$data[simbuho]=$row[simbuho]; //심사번호
	$sabunho=explode("-",$row[school2]);
	//$sabunho[2]=substr($sabunho[2], 0, 1); 
	$sabun=$sabunho[0]."-".$sabunho[1]."-".$sabunho[2];

	$pbo=$row[school1]." 현장실습 참가자 및 실습기관";
    $data[school1]=iconv("utf-8","euc-kr",$row[school1]); //계약자
	$data[school2]=$sabun; //계약자
	$data[school3]=iconv("utf-8","euc-kr",$pbo)."(".$sabun.")"; //계약자

	$pibo=$row[school3].$row[school1]." / 물건명 교육부고시 대학생  현장실습활동 ";
	$pibo2=$row[school1]." 현장실습 ";
  ...
  //$row[school6]=4;
	...
	$data[sigi2_1]=$sigi2_[0];
	$data[sigi2_2]=$sigi2_[1];
	$data[sigi2_3]=$sigi2_[2];
	...
	$data[school21]=iconv("utf-8","euc-kr",$row[school1]); //계약자
	$data[school22]=iconv("utf-8","euc-kr",$row[school1]); //계약자
...
//$printDay=$toyear."-".$tomonth."-".$today;
$data[today]=iconv("utf-8","euc-kr",$row[school7]);


   // 템플릿 파일명  
  $tpl_file = './frm/claim_3_01.frm'; 
...
if($_p==1){
	 ...
	 $data[school1]=iconv("utf-8","euc-kr",$row[school1]);
