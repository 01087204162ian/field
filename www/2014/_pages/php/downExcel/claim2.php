...
if (file_exists(__DIR__ . '/hanwha.png')) {
    $pdf->Image(__DIR__ . '/hanwha.png', 150, 15, 40, 30); // X=150, Y=15, 폭=40, 높이=30
} else {
    // 이미지가 없으면 텍스트로 표시
...
// 보험 유형 테이블 
// 1행: 보장내용(rowspan=2) + 가입유형 선택(colspan=2)
$pdf->drawCell(15, $currentY, 60, 16, '보장내용', 1, true, 9, 'C'); // 높이 16 (2행 병합)
$pdf->drawCell(75, $currentY, 120, 8, '가입유형 선택(체크기호)', 1, true, 9, 'C'); // 폭 120 (2열 병합)
$currentY += 8;
...
// 보상한도 설정 (directory 값에 따라)
if($row['directory'] == 2) { // 고등학교
    $daein1 = 1; // PLAN A 대인대물
    $daein2 = 1; // PLAN A 산재보험
    $daein3 = 2; // PLAN B 대인대물  
    $daein4 = 2; // PLAN B 산재보험
} else { // 대학교
    $daein1 = 2; // PLAN A 대인대물
    $daein2 = 2; // PLAN A 산재보험
    $daein3 = 3; // PLAN B 대인대물
    $daein4 = 3; // PLAN B 산재보험
}
...
$weekWidth = 25;  // 실습기간 열 폭
$peopleWidth = 20; // 참여인원 열 폭
$x = 15;

for ($i = 0; $i < 8; $i++) {
    $width = ($i % 2 == 0) ? $weekWidth : $peopleWidth; // 짝수: 실습기간, 홀수: 참여인원
