...
if (window.QCM_UPLOAD_SCRIPT_LOADED) {
    ...
    class QCM_UploadManager {
        ...
        validateFile(file, documentType) {
			...
			const allowedTypes = {
				question: ['application/pdf', 'image/jpeg', 'image/jpg', 'image/png', 'image/gif', 'image/bmp'],
				subscription: ['application/pdf', 'image/jpeg', 'image/jpg', 'image/png', 'image/gif', 'image/bmp'],
				department: [
					// Excel 파일
				...
				certificate: ['application/pdf', 'image/jpeg', 'image/jpg', 'image/png', 'image/gif', 'image/bmp'],
				receipt: ['application/pdf', 'image/jpeg', 'image/jpg', 'image/png', 'image/gif', 'image/bmp']
			};

			const allowed = allowedTypes[documentType];
			if (allowed && !allowed.includes(file.type)) {
				throw new Error(`지원되지 않는 파일 형식입니다. 허용된 형식: ${this.getReadableFileTypes(documentType)}`);
			}
        ...
        getDocumentConfig(documentType) {
            const documentConfig = {
               ...
               department: {
						...
						acceptFiles: '.xlsx,.xls,.csv,.pdf,.jpg,.jpeg,.png,.gif,.bmp',
						instructions: '1. 예시파일을 다운로드하거나 직접 작성합니다.<br>2. Excel 형식으로 과별인원 정보를 입력하거나<br>&nbsp;&nbsp;&nbsp;작성된 문서를 스캔/촬영합니다.<br>3. 파일을 업로드합니다.<br><br><strong>💡 지원 형식:</strong><br>• Excel: XLSX, XLS, CSV<br>• 문서: PDF<br>• 이미지: JPG, PNG, GIF, BMP'
					}
            };
		...
		getReadableFileTypes(documentType) {
				const readableTypes = {
					question: 'PDF, JPG, PNG, GIF, BMP',
					subscription: 'PDF, JPG, PNG, GIF, BMP',
					department: 'XLSX, XLS, CSV, PDF, JPG, PNG, GIF, BMP',
					certificate: 'PDF, JPG, PNG, GIF, BMP',
					receipt: 'PDF, JPG, PNG, GIF, BMP'
				};
				
				return readableTypes[documentType] || '지원되는 파일 형식';
			}
    }
